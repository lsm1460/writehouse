...
- 넉넉히주고 F만들어서 다 소모시키도록 하는 방도 만들어보자
- 시작 시 나레이션? 스토리용 아이템 추가? npc?
- F + I = A? A + \_ = 8? A는 무엇으로 하지? Ash? Air? Active <-> s?
- I+3 > B -> 3턴 뒤 터지는 폭탄
- 8+I > % -> 무선 에너지 송수신기? 반경 3까지 L과 1에 전력 공급
- v+I > Y 상하좌우 몬스터 발견 시 전기 발생
- D 대각선으로 1칸 이내에 전기타일이 있고, 전기타일이 활성화되어있다면 좌우로 이동, 상하에 활성화된 전기타일이 있다면 멈춤
  상하에 활성화되지 않은 전기타일이 있다면 좌우로 1칸 이동
- 몬스터 순찰
